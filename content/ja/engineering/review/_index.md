...
原則、すでに承認をもらっていれば、再レビューの依頼は不要です。
ただし、承認をもらっていても確認してもらいたい場合は、再度レビューを依頼することができます。

また、Request changes状態で、修正コミットを行った場合は、再度レビューを依頼することが必要です。

プルリクエストのレビュワーリストから `Re-request review` ボタンを押すことで未レビュー状態に戻すことが可能です。

この場合はレビュワーに通知が飛ばないため、メンションで再レビュー依頼を行うことが好ましいです。
